...
object Day6 extends IOApp {
  ...
  case class Context(turnOn: Long => Long, turnOff: Long => Long, toggle: Long => Long)

  object Runner extends Day[List[SantaInstruction], Context, Long](2015, 6) {
    ...
    override def process(input: List[SantaInstruction], context: Option[Context]): Option[Long] =
      context.map { ctx =>
