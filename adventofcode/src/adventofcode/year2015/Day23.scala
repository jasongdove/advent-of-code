...
case class Day23Context(startingRegisters: Map[Char, Int])

object Day23 extends Day[List[AsmInstruction], Day23Context, Int](2015, 23) {

  override def transformInput(lines: List[String]): List[AsmInstruction] =
    lines.map(AsmInstruction.from)

  override def partOneContext(): Option[Day23Context] =
    Some(Day23Context(Map.empty))

  override def partTwoContext(): Option[Day23Context] =
    Some(Day23Context(Map('a' -> 1)))

  override def process(input: List[AsmInstruction], context: Option[Day23Context]): Option[Int] =
    context.flatMap { ctx =>
